...
class Device {
  ...
  def carrierId: Int = this._carrierId
  def userAgent: String = this._userAgent

  def isDocomo(): Boolean = { this.carrierId == Device.CARRIER_ID_DOCOMO }
  def isAu(): Boolean = { this.carrierId == Device.CARRIER_ID_AU }
  ...
  def loadUserAgent(userAgent: String) {
    ...
    this._carrierId = this.userAgent match {
...
object Device {
  ...
  def apply() = new Device()

  def apply(userAgent:String) = new Device(userAgent)

  def apply(request:HttpServletRequest) = new Device(request)
}
