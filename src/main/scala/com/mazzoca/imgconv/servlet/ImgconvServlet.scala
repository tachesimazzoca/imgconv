...
class ImgconvServlet extends HttpServlet {
  ...
  override def doGet(request: HttpServletRequest, response: HttpServletResponse) {
    ...
    val (
      validURL: Boolean, noTransfer: Boolean,
      backendName: String, filename: String, suffix: String
    ) = ptn findFirstIn request.getPathInfo() match {
    ...
    var sourceOpt: Option[File] = None 
    for (fm <- formats if sourceOpt.isEmpty) {
      sourceOpt = uf.fetch(filename + "." + fm, new Date().getTime() - maxage)
    }
    ...
    try {
      ...
      DefaultPluginBrokerFactory.create(cvopt).execute(new FileInputStream(sourceOpt.get), baos)
    ...
    var buf = new Array[Byte](BUFFER_SIZE) 
    ...
    try {
      ...
      var n = 0 
      while ({n = is.read(buf, 0, BUFFER_SIZE); n > 0}) { os.write(buf, 0, n) }
